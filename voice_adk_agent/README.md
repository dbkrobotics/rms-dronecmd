...
2. `server.py` captures server camera (`/dev/video0`) frames in memory (JPEG, no file save).
...
3. Click `Start Mic`.
4. Speak command.
5. Say `confirm` to execute staged motion command (`cancel` to drop).

## Notes

- Camera is fixed to server-side `/dev/video0`.
